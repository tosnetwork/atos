...
const identityBindingOperationColumns = `id, principal_id, type, checkpoint, idempotency_key, agent_id, reason_code,
	binding_ref, content_hash, failure_reason, created_at, completed_at, updated_at`
...
func identityBindingOperationWriteArgs(op domain.IdentityBindingOperation) []any {
	return []any{
		...
		op.BindingRef, identityBindingOperationContentHash(op), op.FailureReason,
...
const upsertIdentityBindingOperationSQL = `
	...
	ON CONFLICT (id) DO UPDATE SET
		checkpoint=$4, binding_ref=$8, failure_reason=$10, completed_at=$12, updated_at=$13
`
...
func scanIdentityBindingOperation(row pgx.Row) (domain.IdentityBindingOperation, error) {
	...
	if err := row.Scan(
		...
		&op.BindingRef, &contentHash, &op.FailureReason, &op.CreatedAt, &op.CompletedAt, &op.UpdatedAt,
	); err != nil {
