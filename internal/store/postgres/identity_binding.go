...
const identityBindingOperationColumns = `id, principal_id, type, checkpoint, idempotency_key, agent_id, reason_code,
	binding_ref, ref_network, created, revoked, content_hash, failure_reason, created_at, completed_at, updated_at`
...
func identityBindingOperationWriteArgs(op domain.IdentityBindingOperation) []any {
	return []any{
		...
		op.BindingRef, op.RefNetwork, op.Created, op.Revoked, identityBindingOperationContentHash(op), op.FailureReason,
...
const upsertIdentityBindingOperationSQL = `
	...
	ON CONFLICT (id) DO UPDATE SET
		checkpoint=$4, binding_ref=$8, ref_network=$9, created=$10, revoked=$11, failure_reason=$13, completed_at=$15, updated_at=$16
`
...
func scanIdentityBindingOperation(row pgx.Row) (domain.IdentityBindingOperation, error) {
	...
	if err := row.Scan(
		...
		&op.BindingRef, &op.RefNetwork, &op.Created, &op.Revoked, &contentHash, &op.FailureReason, &op.CreatedAt, &op.CompletedAt, &op.UpdatedAt,
	); err != nil {
...
func (s *Store) LatestCompletedIdentityBindingOperation(ctx context.Context, principalID string) (domain.IdentityBindingOperation, bool, error) {
	op, err := scanIdentityBindingOperation(s.pool.QueryRow(ctx, `
		SELECT `+identityBindingOperationColumns+` FROM identity_binding_operations
		WHERE principal_id=$1 AND checkpoint='completed'
