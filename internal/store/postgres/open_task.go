...
func (s *Store) UpdateAcceptanceOperation(ctx context.Context, id string, fn func(domain.AcceptanceOperation, bool) (domain.AcceptanceOperation, error)) (domain.AcceptanceOperation, error) {
	...
	// checkpoint transition with the OpenTask projection/reopen in one
	// transaction -- a plain UpdateAcceptanceOperation call reaching
	// either terminal checkpoint would recreate exactly the split-commit
	// crash window those two methods exist to close. Enforced here, not
	// just documented on the interface, so a future caller mistake fails
