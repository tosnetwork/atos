...
func (s *Store) UpdateAcceptanceOperation(ctx context.Context, id string, fn func(domain.AcceptanceOperation, bool) (domain.AcceptanceOperation, error)) (domain.AcceptanceOperation, error) {
	...
	// projection/reopen in one transaction -- a plain
	// UpdateAcceptanceOperation call pushing a STILL-non-terminal
	// operation INTO a terminal checkpoint would recreate exactly the
	// split-commit crash window those two methods exist to close.
