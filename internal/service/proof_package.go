...
// The Phase 4B-1 canonical commitment binds the provider-issued service quote
// identity. UnderlyingServiceQuoteRef is the later chain observation reference
// and must not replace that immutable business identity in a portable proof.
func proofServiceQuoteRef(q domain.Quote) string { return q.ServiceQuoteID }
