...
func TestPhase3B_EndToEndProviderTrustReadinessAcceptance(t *testing.T) {
	...
	granted, reasonCode, err := capabilities.EvaluateActivation(ctx, service.FailClosedActivationAuthority{}, "prn_e2e_admin", cap.ID, domain.TrustModeVerified, "eval-e2e-step10-"+providerID)
	if err != nil {
		t.Fatal(err)
	}
