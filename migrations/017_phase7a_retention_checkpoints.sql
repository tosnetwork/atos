ALTER TABLE financial_batches
    ADD COLUMN IF NOT EXISTS required_retain_until TIMESTAMPTZ,
    ADD COLUMN IF NOT EXISTS anchor_required_retain_until TIMESTAMPTZ,
    ADD COLUMN IF NOT EXISTS anchor_retained_object_key TEXT NOT NULL DEFAULT '',
    ADD COLUMN IF NOT EXISTS anchor_retained_version_id TEXT NOT NULL DEFAULT '';
...
BEGIN
    ...
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.required_retain_until IS NOT NULL AND
       NEW.required_retain_until IS DISTINCT FROM OLD.required_retain_until THEN
        RAISE EXCEPTION 'financial batch retention deadline is immutable';
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.anchor_required_retain_until IS NOT NULL AND
       NEW.anchor_required_retain_until IS DISTINCT FROM OLD.anchor_required_retain_until THEN
        RAISE EXCEPTION 'financial anchor receipt retention deadline is immutable';
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.anchor_retained_object_key <> '' AND (
       NEW.anchor_retained_object_key IS DISTINCT FROM OLD.anchor_retained_object_key OR
       NEW.anchor_retained_version_id IS DISTINCT FROM OLD.anchor_retained_version_id) THEN
        RAISE EXCEPTION 'retained financial anchor receipt identity is immutable';
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.state IN ('retained','anchored') AND (
        ...
        NEW.required_retain_until IS DISTINCT FROM OLD.required_retain_until OR
    ...
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.state = 'signed' AND NEW.state = 'retained' AND
       (NEW.required_retain_until IS NULL OR NEW.retained_object_key = '' OR NEW.retained_version_id = '') THEN
        RAISE EXCEPTION 'financial batch retention transition requires a durable deadline and exact object version';
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.state = 'anchored' AND (
       NEW.anchor_id IS DISTINCT FROM OLD.anchor_id OR
       NEW.anchor_required_retain_until IS DISTINCT FROM OLD.anchor_required_retain_until OR
       NEW.anchor_retained_object_key IS DISTINCT FROM OLD.anchor_retained_object_key OR
       NEW.anchor_retained_version_id IS DISTINCT FROM OLD.anchor_retained_version_id) THEN
    ...
    IF TG_OP = 'UPDATE' AND OLD.state = 'retained' AND NEW.state = 'anchored' AND
       (OLD.anchor_id <> '' OR NEW.anchor_id = '' OR NEW.anchor_required_retain_until IS NULL OR
        NEW.anchor_retained_object_key = '' OR NEW.anchor_retained_version_id = '') THEN
        RAISE EXCEPTION 'financial batch anchor transition requires one immutable retained anchor receipt';
    END IF;
